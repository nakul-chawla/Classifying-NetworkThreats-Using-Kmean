...
struct data initial_data[1000];				//For data
...
struct data cluster1[1000];
struct data cluster2[1000];
struct data cluster3[1000];
...
void train()
{
	printf("This application will classify the input data into:-\n\n KNOWN-THREAT(-1)  UNKNOWN-THREAT(-2)  NO-THREAT(1)\n\nOn the basis of 5 features:-\n\nSource_Bytes\tDestination_Bytes\tSource_Port_Number\tDestination_Port_Number\t  Protocol\n\nFirst our application has to be trained.");
	printf("\n\nEnter the location of the file to train : ");	
	readData();
	initialize_mean();
	distance(initial_data,total_rows);
	calculate_new_mean();	
	...
	{
	printf("\n\nCluster: %d",i);
	printf("\t%d %d %d %d %d",mean[i].sport,mean[i].dport,mean[i].sbytes,mean[i].dbytes,mean[i].protocol);
	}

}

struct data test_value = {0, 0,63758,11572,1,-1};
...
int main()
{
	...
	printf("\n\nCluster is:%d\n\nLabel is:%d\n",cluster_number+1,cluster_labels[cluster_number]);
	//cluster();
	return 0;
}
